...
class EarlyReflections:
    def __init__(self, source: Point3D, mic: Point3D, reflection_points: list[Point3D], sim_config: SimulationConfig, room_config: RoomConfig):
        ...
        delay_times = self._make_delays()
        
        # attenuation over distance (1/r law)
        distance_attenuation = self._make_attenuation()
        
        # TODO - add wall attenuation gain / filter 
        self.tapped_delay_line = TappedDelayLine(delay_times, distance_attenuation, self.fs) # inject tdl instead
    ...
    def process(self, input_signal, output_signal, type: str):
        ...
        if type == 'convolution': print('convolution')
    
    def _make_delays(self):
        return [self._point_to_delay(self.mic, point) for point in self.reflection_points]
    
    def _make_attenuation(self):
        return [self._point_to_attenuation(self.mic, point) for point in self.reflection_points]
        
    def _point_to_delay(self, mic, point):
